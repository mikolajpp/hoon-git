...
::  making manual thread use almost impossible
::  without some vase cleverness. 
::
+$  args
...
=/  =yuki:clay
  =-  [~ (malt -)]
  %+  turn
    (as-namespace:git-clay repo default-branch dir.args)
  |=  [=path =page]
  ~&  path
  [path &/page]
;<  ~  bind:m  
  (send-raw-card:io %pass /clay %arvo %c %park u.desk.args &/yuki *rang:clay)
(pure:m !>(repo))
